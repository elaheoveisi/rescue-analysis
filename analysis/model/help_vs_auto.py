...
def _score(y_true: list, y_pred: list) -> tuple[float | None, float | None]:
    if len(set(y_true)) < 2:
        return None, None
    return roc_auc_score(y_true, y_pred), accuracy_score(y_true, (np.array(y_pred) > 0.5).astype(int))
...
def run_classifiers(cfg: dict, df: pd.DataFrame) -> pd.DataFrame:
    ...
    rf_n_estimators = hva_cfg.get("rf_n_estimators", 300)
    rf_max_depth = hva_cfg.get("rf_max_depth", 4)
    rf_min_samples_leaf = hva_cfg.get("rf_min_samples_leaf", 5)
    ...
    for w in sorted(df["window"].unique()):
        ...
        for r in range(n_repeats):
            ...
            scores["glmm"].append(_score(*_loso_glmm(bal, zcols)))
            scores["rf"].append(_score(*_loso_sklearn(
                bal, zcols,
                lambda: RandomForestClassifier(
                    n_estimators=rf_n_estimators, max_depth=rf_max_depth,
                    min_samples_leaf=rf_min_samples_leaf, random_state=0,
                ),
            )))
...
def feature_importance(cfg: dict, df: pd.DataFrame) -> pd.DataFrame:
    ...
    features = hva_cfg["features"]
    seed0 = hva_cfg.get("random_state", 42)
    rf_n_estimators = hva_cfg.get("rf_importance_n_estimators", 500)
    rf_max_depth = hva_cfg.get("rf_max_depth", 4)
    rf_min_samples_leaf = hva_cfg.get("rf_min_samples_leaf", 5)
...
def run(cfg: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    return events_df, features_df, results, importance
